<script lang="ts">
    ...
    // Import directly from the environment
    ...
    onMount(async () => {
        ...
        supabaseUrl = data.supabaseInfo.url || 'Not set';
        supabaseKey = PUBLIC_SUPABASE_ANON_KEY ? 'Set (hidden for security)' : 'Not set';
        
        debugInfo += `Direct environment check:\n`;
        debugInfo += `PUBLIC_SUPABASE_URL: ${data.supabaseInfo.url ? data.supabaseInfo.url : 'Not available'}\n`;
        debugInfo += `PUBLIC_SUPABASE_ANON_KEY set: ${data.supabaseInfo.keyAvailable ? 'Yes' : 'No'}\n`;
        ...
        try {
            ...
            if (sessionError) {
                ...
                debugInfo += `Session data: ${JSON.stringify(sessionData)}\n`;
            }
        } catch (e) {
...
<div class="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-lg mt-10">
    <h1 class="text-2xl font-bold mb-6">Authentication Debug Page (Direct Import)</h1>
    
    <div class="mb-6 p-4 bg-gray-100 rounded-lg">
        <h2 class="text-lg font-semibold mb-2">Environment Check</h2>
        <p><strong>Supabase URL:</strong> {data.supabaseInfo.url}</p>
